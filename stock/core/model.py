...
class MLModel:
    ...
    def predict_until(self, steps_ahead):
        last_60 = self.data['Close'].values[-60:]
        scaled_input = self.scaler.transform(last_60.reshape(-1, 1)).flatten().tolist()
        predictions = []

        for _ in range(steps_ahead):
            X_input = np.array(scaled_input[-60:]).reshape(1, 60, 1)
            next_scaled = self.model.predict(X_input)[0][0]
            scaled_input.append(next_scaled)
